...
class Calculator extends Component
{
    ...
    public function calculateHeckenschnitt($f){
        return (($f['hecke_lfm'] ?? 0) * ($f['hecke_freq'] ?? 0) * 5.0);
    }

    public function calculateHofpflege($f){
        return (($f['hof_groesse'] ?? 0) * ($f['hof_freq'] ?? 0) * 2.0);
    }

    public function calculateDachrinnenreinigung($f){
        return (($f['dachrinne_lfm'] ?? 0) * ($f['dachrinne_freq'] ?? 0) * 5.0);
    }

    public function calculateFensterreinigung($f){
        return (($f['fenster_anzahl'] ?? 0) * ($f['fenster_freq'] ?? 0) * 7.5);
    }
